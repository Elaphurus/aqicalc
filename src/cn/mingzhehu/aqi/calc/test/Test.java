package cn.mingzhehu.aqi.calc.test;

import java.util.ArrayList;
import java.util.HashMap;

import cn.mingzhehu.aqi.calc.AqiCalc;
import cn.mingzhehu.aqi.calc.DailyCalc;
import cn.mingzhehu.aqi.calc.RealTimeCalc;
...
public class Test {
	...
	private static void testDaily() {
		...
		int iAqi_so2 = dailyCalc.getIAqi("so2", 50);
		indexMap.put("so2", iAqi_so2);
		int iAqi_no2 = dailyCalc.getIAqi("no2", 80);
		indexMap.put("no2", iAqi_no2);
		int iAqi_pm10 = dailyCalc.getIAqi("pm10", 250);
		indexMap.put("pm10", iAqi_pm10);
		int iAqi_co = dailyCalc.getIAqi("co", 4000);
		indexMap.put("co", iAqi_co);
		int iAqi_o3 = dailyCalc.getIAqi("o3", 100);
		indexMap.put("o3", iAqi_o3);
		int iAqi_pm25 = dailyCalc.getIAqi("pm25", 35);
